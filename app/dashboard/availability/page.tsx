...
export default function AvailabilityPage() {
  ...
  const [teamAvailability, setTeamAvailability] = useState<AvailabilityRecord[]>([]);
  ...
  const isAdmin = userData?.role === 'líder' || userData?.email === 'pedrohenriqueribei@gmail.com';
    ...
    <div className="max-w-4xl mx-auto space-y-10 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-display font-black text-slate-800 dark:text-slate-100 tracking-tight flex items-center gap-3">
            ...
            {showTeamView ? 'Disponibilidade da Equipe' : 'Minha Disponibilidade'}
          </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          {isAdmin && (
            ...
            >
              {showTeamView ? <Eye className="w-4 h-4" /> : <Users className="w-4 h-4" />}
        ...
        {(loading || (showTeamView && teamLoading)) ? (
          ...
          <div className="relative z-10">
            <div className="grid grid-cols-7 mb-4">
              {weekDays.map(day => (
                <div key={day} className="text-center text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest py-4">
                  {day}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-3 md:gap-4">
              ...
              {Array.from({ length: daysInMonth }).map((_, idx) => {
                ...
                const availableCount = showTeamView ? getAvailableCount(day) : 0;
                ...
                return (
                  <motion.button
                    ...
                    className={`
                      ...
                      ${showTeamView 
                        ? (availableCount > 0 
                            ? (isDetailSelected ? 'bg-indigo-700 border-indigo-700 text-white shadow-lg' : 'bg-indigo-50 border-indigo-100 text-indigo-700 hover:bg-indigo-100')
                    ...
                    `}
                  >
                    <span className={`text-xl font-black ${isSelected || isDetailSelected ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}>
                      {day}
                    </span>
                    {showTeamView && availableCount > 0 && (
                      <span className={`text-[10px] font-black ${isDetailSelected ? 'text-white/80' : 'text-indigo-400'}`}>
                        {availableCount}
                      </span>
                    )}
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="font-display font-black text-indigo-900 tracking-tight">
                       Disponíveis no dia {selectedDayDetail} de {monthNames[month]}
                    </h4>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-4">
          <div className="flex items-center gap-6 bg-slate-100/50 dark:bg-slate-800/30 px-8 py-4 rounded-3xl border border-white dark:border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-500">Disponível</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-md"></div>
              <span className="text-xs font-bold text-slate-500">Indisponível</span>
            </div>
      ...
      {showTeamView && (
        <div className="p-8 bg-indigo-50 border border-indigo-100 rounded-[2.5rem] flex gap-6">
          ...
          <div>
            <h4 className="font-bold text-indigo-900 mb-1 tracking-tight">Visão Geral da Equipe</h4>
            <p className="text-sm text-indigo-700/80 leading-relaxed">
              Clique em um dia destacado para ver quais integrantes estão disponíveis. Isso facilita a montagem das escalas de cada final de semana.
            </p>
          </div>
        </div>
      )}
      ...
      {!showTeamView && (
        <div className="p-8 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-[2.5rem] flex gap-6">
          ...
          <div>
            <h4 className="font-bold text-amber-800 dark:text-amber-400 mb-1 tracking-tight">Lembrete Importante</h4>
            <p className="text-sm text-amber-700/80 dark:text-amber-400/60 leading-relaxed">
              Sua disponibilidade ajuda os líderes a planejarem as escalas de forma justa e organizada. 
              Tente atualizar seus dias sempre que houver mudanças em sua rotina mensal.
            </p>
          </div>
        </div>
      )}
