  ...
  if (!church) {
    return (
      <div className="text-center py-20 bg-white rounded-[3rem] border border-slate-100">
        <Church className="w-16 h-16 text-slate-200 mx-auto mb-6" />
        <h2 className="text-2xl font-bold text-slate-800 mb-4">Igreja não encontrada</h2>
        <button onClick={() => router.back()} className="text-blue-800 font-bold flex items-center gap-2 mx-auto">
  ...
  return (
    <div className="max-w-6xl space-y-12 pb-20">
      <header className="flex items-center justify-between">
        <button 
          onClick={() => router.back()}
          className="flex items-center gap-2 text-slate-400 hover:text-blue-800 font-bold transition-colors group"
        >
          <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center border border-slate-100 group-hover:border-blue-100 group-hover:shadow-lg transition-all">
      ...
      <section className="bg-white rounded-[4rem] p-12 border border-slate-100 shadow-sm relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-blue-50 blur-[120px] -mr-48 -mt-48 opacity-50"></div>
        <div className="relative z-10 flex flex-col md:flex-row gap-12 items-start md:items-center">
          ...
          <div className="flex-1 space-y-4">
            <div className="flex flex-wrap gap-3">
              <span className="px-4 py-1.5 bg-blue-50 text-blue-800 rounded-full text-[10px] font-black uppercase tracking-widest border border-blue-100">
                Ministério de Louvor
              </span>
            </div>
            <h1 className="text-4xl md:text-5xl font-display font-black text-slate-800 tracking-tighter leading-tight">
              {church.name}
            </h1>
            <div className="flex flex-wrap gap-8 pt-4">
              <div className="flex items-center gap-3 text-slate-500 font-medium">
                <PastorIcon className="w-5 h-5 text-blue-400" />
                <span>{church.pastor || 'Pastor não informado'}</span>
              </div>
              <div className="flex items-start gap-3 text-slate-500 font-medium max-w-md">
                <MapPin className="w-5 h-5 text-blue-400 mt-0.5 shrink-0" />
                <span>{church.address || 'Endereço não informado'}</span>
              </div>
            </div>
          </div>
          <div className="w-full md:w-auto flex flex-col items-center justify-center p-8 bg-slate-50 rounded-[3rem] border border-white">
            <div className="text-4xl font-display font-black text-blue-800 mb-1">{members.length}</div>
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Integrantes</div>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        <div className="space-y-8">
          <h2 className="text-2xl font-display font-black text-slate-800 flex items-center gap-4">
          ...
          <div className="space-y-6">
            {Object.keys(groupedData).length > 0 ? (
              Object.entries(groupedData).map(([group, list]) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="font-display font-black text-slate-800 text-lg capitalize">{group}</h3>
                    <span className="ml-auto text-[10px] font-bold text-slate-300 bg-slate-50 px-3 py-1 rounded-full">{list.length}</span>
                  </div>
                  ...
                  <div className="space-y-4">
                    {list.map(m => (
                      <Link 
                        ...
                        className="group flex items-center gap-4 p-4 rounded-[2rem] hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100"
                      >
                        <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-400 group-hover:bg-white group-hover:text-blue-800 transition-colors">
                          {m.name.charAt(0)}
                        </div>
                        <div className="flex-1">
                          <div className="font-bold text-slate-800 group-hover:text-blue-800 transition-colors">{m.name}</div>
                          <div className="text-xs text-slate-400 capitalize">{m.role} {m.level ? `• ${m.level}` : ''}</div>
                        </div>
                        <div className="w-10 h-10 rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all bg-white border border-slate-100 text-blue-800 shadow-sm">
              ...
              <div className="bg-slate-50 rounded-[3rem] p-12 text-center border border-dashed border-slate-200">
                ...
                <p className="text-slate-500 font-medium">Nenhum integrante vinculado a esta igreja ainda.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <section className="bg-blue-600 rounded-[3.5rem] p-12 text-white shadow-2xl shadow-blue-500/20 relative overflow-hidden">
           ...
           <div className="relative z-10">
              ...
              <p className="text-blue-100 text-sm mb-10 leading-relaxed font-medium">
                Deseja adicionar mais músicos ou vocalistas a esta igreja? Acesse a área de membros para realizar o cadastro.
              </p>
