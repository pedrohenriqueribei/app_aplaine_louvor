  ...
  return (
    <div className="max-w-4xl space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-display font-bold text-slate-800 dark:text-slate-100">Notificações</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Acompanhe suas convocações e avisos</p>
        </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="p-20 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[3rem] flex flex-col items-center text-center">
            ...
            <p className="text-slate-500 mt-2">Você não possui novas notificações no momento.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex gap-6 items-start">
                  <div className={`mt-1 p-3 rounded-2xl ${
                    notif.read ? 'bg-slate-100 dark:bg-slate-800' : 'bg-blue-50 dark:bg-blue-900/40'
                  }`}>
                    {notif.type === 'schedule' ? (
                      <Calendar className={`w-5 h-5 ${notif.read ? 'text-slate-400' : 'text-blue-800 dark:text-blue-400'}`} />
                    ) : (
                      <Bell className={`w-5 h-5 ${notif.read ? 'text-slate-400' : 'text-blue-800 dark:text-blue-400'}`} />
                    )}
                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <h4 className={`font-bold transition-colors ${notif.read ? 'text-slate-600 dark:text-slate-400' : 'text-slate-900 dark:text-slate-100'}`}>
                        {notif.title}
                      </h4>
                    ...
                    <p className={`mt-1 text-sm leading-relaxed ${notif.read ? 'text-slate-500 dark:text-slate-500' : 'text-slate-600 dark:text-slate-400'}`}>
                      {notif.body}
                    </p>
                    {!notif.read && (
