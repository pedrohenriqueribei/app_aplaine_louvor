...
export default function SongsPage() {
  ...
  const fetchOriginalKey = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Erro na consulta com o servidor.");
      }
      ...
      alert(
        error.message ||
          "Ocorreu um erro ao buscar os dados da música. Tente novamente em instantes.",
      );
    } finally {
  ...
  const fetchChordSheet = async () => {
    ...
    try {
      ...
      alert(
        error.message || "Erro ao buscar a cifra. Por favor, tente novamente.",
      );
    } finally {
    ...
    <div className="space-y-12">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-900 h-64 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 animate-pulse"
            ></div>
          ))}
        </div>
          ...
          {filteredSongs.map((song, idx) => (
            ...
            >
              <div className="p-8">
                ...
                <p className="text-sm font-bold text-slate-400 dark:text-slate-500 mb-6 truncate">
                  {song.artist}
                </p>

                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-slate-100 dark:border-slate-800">
                    <div className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-1">
                      Tom Pessoal
                    </div>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {song.tags?.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-full text-[9px] font-black uppercase tracking-wider border border-emerald-100 dark:border-emerald-800"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div className="bg-white dark:bg-slate-900 p-20 rounded-[3rem] border border-slate-200 dark:border-slate-800 border-dashed flex flex-col items-center text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-sm mb-8">
            Comece adicionando as músicas que você toca ou canta.
          </p>
          <button
      ...
      {viewingSong && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-4xl bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-10">
              ...
              {viewingSong.tags && viewingSong.tags.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {viewingSong.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-4 py-2 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-slate-100 dark:border-slate-800"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              {viewingSong.observations && (
                <section className="space-y-4">
                  <h3 className="text-xs font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">
                    Observações
                  </h3>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-3xl bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl p-8 md:p-12 overflow-y-auto max-h-[90vh]">
            <button
              onClick={() => setIsModalOpen(false)}
            ...
            <p className="text-slate-400 dark:text-slate-500 font-medium mb-10">
              Cadastre a música em seu tom pessoal para organizar seu
              repertório.
            </p>
            ...
            <form onSubmit={handleSave} className="space-y-8">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center pr-2">
                    ...
                    <button
                      ...
                      disabled={
                        searchingKey || !formData.title || !formData.artist
                      }
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={
                      searchingSheet || !formData.title || !formData.artist
                    }
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest pl-1">
                  Observações
                </label>
                <textarea
                  rows={3}
